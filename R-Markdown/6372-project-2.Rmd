...
```{r}
#fitting a simple model by using only 4 predictors which appeared to be significant in EDA
        simple.logistics<-glm(y~euribor3m + cons.price.idx + poutcome + cons.conf.idx, family="binomial",data=bank.additional.sample)
...
```{r}
#Objective 2 (Point 3) -- LDA on Sample data
bank.additional.lda <- lda (y ~ ., bank.additional.sample)
bank.additional.lda.p <- predict(bank.additional.lda, test)$class
test <- as.data.frame(test)
table(bank.additional.lda.p, test[,21])

bank.additional.full <- as.data.frame(bank.additional.full)
bank.additional.lda.cv <- lda (y ~ ., bank.additional.full, CV=TRUE)
table(bank.additional.lda.cv$class, bank.additional.full[,21])
```
...
for(i in 1:50)
{
  ...
  table(test$y,classifications)
  CM = confusionMatrix(table(test$y,classifications))
...
# k = 32
classifications = knn(train[,c(17,19)],test[,c(17,19)],train$y, prob = TRUE, k = 32)
table(test$y,classifications)
confusionMatrix(table(test$y,classifications))
